#!/usr/bin/env python3
# E03 — Warmup then full fine-tune
...
# Niceties
# -----------------------
# Respect Slurm CPU threads if set
...
opt2 = tf.keras.optimizers.Adam(learning_rate=LR_FINETUNE, clipnorm=1.0)  # fresh optimizer
...
cbs2 = [
    ...
    tf.keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.2,
                                         patience=1, min_lr=1e-6, verbose=1),
    tf.keras.callbacks.CSVLogger(str(CSV_OUT), append=True),  # append to same CSV
    t2,
]
...
meta = {
    ...
    "seed": SEED,
    "stamp": stamp,
    "notes": "E03 full fine-tune after warmup; no augmentation."
}
