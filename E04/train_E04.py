#!/usr/bin/env python3
# E04 — Light augmentation + full fine-tune
...
BUNDLE   = OUT_DIR / "bundles" / "model_E04"   

MODEL_OUT = BUNDLE / "model_E04.keras"
CSV_OUT   = BUNDLE / "train_log.csv" 
META_OUT  = BUNDLE / "run_E04_meta.json"
...
print("[E04] Warmup head-only for", WARMUP_EPOCHS, "epochs @", LR_WARMUP)
model.fit(train_ds, validation_data=val_ds, epochs=WARMUP_EPOCHS, callbacks=cbs1, verbose=2)

# Phase 2 — unfreeze ALL, fresh optimizer with clipnorm
print("[E04] Unfreezing ALL layers, fine-tuning @", LR_FINETUNE, "(clipnorm=1.0)")
...
meta = {
    ...
    "augmentation": "flip+brightness±5%+contrast±5%",
    ...
    "notes": "E04: light augmentation; full-FT after warmup."
}
